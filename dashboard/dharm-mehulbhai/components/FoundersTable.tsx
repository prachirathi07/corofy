...
export default function FoundersTable() {
  ...
  const calculatePriority = (founder: Founder) => {
    ...
    // Get mail_replies from raw data (since we removed it from mapping)
    const mailReplys = (founder as any).mail_replies;
    ...
    // Get followup data from raw data (since we removed it from mapping)
    const followUp5Days = (founder as any).followup_5_sent;
    const followUp10Days = (founder as any).followup_10_sent;
    ...
    // Check if mail was sent (handle various status formats)
    const isMailSent = mailStatus && (
      mailStatus.toLowerCase() === 'sent' ||
      mailStatus.toLowerCase() === 'email_sent' ||
      mailStatus.toLowerCase().includes('sent')
    );

    // For mail sent but no reply yet, or follow-ups sent - show "-" instead of Medium
    if (isMailSent || followUp5Days || followUp10Days) {
  ...
  // Filter founders by search query and priority
  const filteredFounders = founders.filter(founder => {
    // Search filter
    if (searchQuery) {
      const query = searchQuery.toLowerCase();
      const matchesSearch = (
        (founder['Founder Name']?.toLowerCase() || '').includes(query) ||
        (founder['Company Name']?.toLowerCase() || '').includes(query) ||
        (founder['Founder Email']?.toLowerCase() || '').includes(query) ||
        (founder['Position']?.toLowerCase() || '').includes(query)
      );
      if (!matchesSearch) return false;
    }

    // Priority filter
    if (tablePriorityFilter) {
      const priority = founder['Priority based on Reply'];
  ...
  const currentFounders = filteredFounders.slice(startIndex, endIndex);
  ...
  const getVerifiedFounders = () => {
    return founders.filter(f => {
      ...
      // Check if mail was sent (handle various status formats)
      const isSent = mailStatus && (
        mailStatus.toLowerCase() === 'sent' ||
        mailStatus.toLowerCase() === 'email_sent' ||
        mailStatus.toLowerCase().includes('sent')
      );
      const isNotSent = !isSent;
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg">
      {/* Header */}
      <div className="bg-blue-600 text-white p-6 rounded-t-lg flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            type="text"
            className="block w-full pl-10 pr-3 py-2 border border-blue-500 rounded-lg leading-5 bg-blue-700 text-white placeholder-blue-200 focus:outline-none focus:bg-blue-800 focus:border-blue-400 focus:placeholder-blue-300 focus:ring-1 focus:ring-blue-400 sm:text-sm transition duration-150 ease-in-out"
      ...
      <div className="p-6">
                  ...
                  <tr>
                    <td colSpan={13} className="px-3 py-4 text-center text-gray-500">
                      {searchQuery
                        ...
                        : tablePriorityFilter
                          ? `No founders found with ${tablePriorityFilter}`
                          : 'No founders found'}
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={founder.id} className={`transition-all duration-150 hover:bg-blue-50/50 hover:shadow-sm ${selectedLeads.has(founder.id) ? 'bg-blue-50 border-l-4 border-blue-500' : 'border-l-4 border-transparent'}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {(() => {
                          ...
                          const statusLower = mailStatus.toLowerCase().trim();
                          const isSent = statusLower === 'sent' ||
                            statusLower === 'email_sent' ||
                            statusLower.includes('sent');
                          const isFailed = statusLower === 'failed';
                          const isReplyReceived = statusLower === 'reply_received' || statusLower.includes('reply');
                          const isFollowup = statusLower.includes('followup');

                          let statusClass = 'bg-yellow-100 text-yellow-800';
                          let statusDot = 'bg-yellow-500';
                          let statusText = mailStatus;
                          ...
                          return (
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium ${statusClass}`}>
                              <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${statusDot}`}></span>
                              {statusText}
                            </span>
                          );
                        })()}
        ...
        {/* Pagination Controls */}
        {!isLoading && !error && filteredFounders.length > 0 && (
          <div className="mt-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <div className="text-sm text-gray-600">
                Showing {startIndex + 1} to {Math.min(endIndex, filteredFounders.length)} of {filteredFounders.length} founder{filteredFounders.length !== 1 ? 's' : ''}
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 pt-0.5">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">
                  Successfully sent emails to <span className="font-semibold text-green-700">400</span> verified founders.
                </p>
